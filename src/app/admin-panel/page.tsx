...
export default function AdminPanel() {
 ...
 const [importProgress, setImportProgress] = useState({
  ...
  const fetchCities = async () => {
    ...
    try {
      const response = await axios.get('https://mts-r-server.onrender.com/api/tariffs-full')
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      const response = await axios.post('https://mts-r-server.onrender.com/api/login', loginData)
  ...
  const handleUpdateTariff = async (id: number) => {
    ...
    try {
      await axios.put(
        `https://mts-r-server.onrender.com/api/tariffs/${selectedCity}/${selectedService}/${id}`,
        editForm
      )
      await fetchCities()
      cancelEditing()
  ...
  const handleMassDelete = async () => {
    ...
    try {
      ...
      await axios.delete('https://mts-r-server.onrender.com/api/tariffs/mass-delete', {
...
const handleExcelImport = async (file: File) => {
      ...
      for (let i = 0; i < cityKeys.length; i++) {
        ...
        for (const serviceKey of serviceKeys) {
          ...
          for (let j = 0; j < tariffs.length; j += IMPORT_CONFIG.chunkSize) {
            ...
            try {
              await axios.post('https://mts-r-server.onrender.com/api/upload-tariffs', chunkData, {
  ...
  const handleMassHide = async (hidden: boolean) => {
    ...
    try {
      const tariffIds = Array.from(selectedTariffs).map(id => String(id))
      await axios.patch('https://mts-r-server.onrender.com/api/tariffs/mass-hide', {
        city: selectedCity,
 ...
 const handleAddTariff = async () => {
  ...
  mobileMinutes: 0
    ...
    await axios.post(
      `https://mts-r-server.onrender.com/api/tariffs/${selectedCity}/${selectedService}`,
      newTariff
    )
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    setLoadingStates(prev => ({...prev, updating: true}))
                                    await axios.patch(
                                      `https://mts-r-server.onrender.com/api/tariffs/${selectedCity}/${selectedService}/${t.id}/hide`,
                                      { hidden: !t.hidden }
                                    )
                                    await fetchCities()
